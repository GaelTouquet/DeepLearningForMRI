...
def preprocess_data(path, name, cat, input_mask=None, output_mask=None,multicoil=False):
    ...
    for f in files:
        ...
        for i,slic in enumerate(h5f['kspace']):
            index_list.append(['D:\\NN_DATA\\{}\\{}\\{}'.format(name,cat,f),i])
            inputs[i] = image_from_kspace(slic,multicoil=multicoil,mask=input_mask)
            outputs[i] = image_from_kspace(slic,multicoil=multicoil,mask=output_mask)
        h5f.close()
        outfile = h5py.File('D:\\NN_DATA\\{}\\{}\\{}'.format(name,cat,f),'w')
...
class DataGenerator(Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        y = np.empty((self.batch_size, *self.dim, self.n_channels), dtype=np.float)
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            y[i] = np.reshape(f['outputs'][ID[1]],(*self.dim,self.n_channels))
        f.close()
